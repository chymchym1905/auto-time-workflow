...
def lambda_handler(event, context):
    ...
    assert len(videos) == len(
        numchamber
    ), "Number of videos and number of chambers must be equal"
    yaml_data = {
        "jobs": {
            "CloneRepo": {
                "resources": {"instance-type": "C4"},
            ...
            "Infer": {
                "resources": {"instance-type": inferinstance},
                "needs": ["CloneRepo"],
                "inputs": {"repo": "CloneRepo.outputs.repo"},
                "outputs": {
                    "timedData": {"type": "dataset", "with": {"ref": datasetversion}}
                },
                "uses": "script@v1",
                "with": {
    ...
    yaml_string = yaml.dump(
        yaml_data, default_flow_style=False, width=float("inf"), indent=2
    )
    workflow_client.run_workflow(
        spec=yaml.safe_load(yaml_string),
        workflow_id="223e903e-3439-412f-ad5b-1f5f005e40b7",
        inputs=None,
    )
    workflow = workflow_client.get(workflow_id="223e903e-3439-412f-ad5b-1f5f005e40b7")

    return json.dumps({"statusCode": 200, "event": event, "body": workflow})
