...
class Chamber:
    ...
    def getTime(self):
        if self.end_frame == 0:
            return "End frame must be greater than 0"
        if self.start_frame == 0:
            return "Start frame must be greater than 0"
...
def find_gaps(objects: list, start, end, numchambers):
    ...
    zero_indices = (
        [start] + [i for i, x in enumerate(objects) if "objective-text" in x] + [end]
    )
    ...
    largest_gap_indices = sorted(
        range(len(gap_lengths)), key=lambda i: gap_lengths[i], reverse=True
    )[:numchambers]
...
def verify_run(object_list, fps, numchambers, title):
    ...
    return chambers, int(round(totalTime, 3) * 1000)
...
def timeruns(num_chambers: list[int], infres):
    times = []
    for video, framedata, numchamber in zip(
        os.listdir("downloads"), infres, num_chambers
    ):
        try:
            objects_present = framedata
            res = verify_run(
                objects_present,
                int(get_fps(os.path.join("downloads", video))),
                int(numchamber),
                video,
            )
...
def timerunsv2(num, filename, filenamewithsegment, inferes, save=True):
    # try:
